...
export default function UnlockClient({ session, state }: UnlockClientProps) {
  ...
  const handleUnlock = async (tier: 'vip' | 'pro' | 'promo') => {
    setLoading(true)
    setError('')
    try {
      ...
      <div className="relative z-10 w-full max-w-5xl space-y-12 animate-auth-fade">
        ...
        {state === 'expired' && (
          <div className="text-center space-y-6">
            ...
            <p className="mx-auto max-w-lg text-[1.05rem] leading-relaxed text-surface-400">
              Renove agora e continue com acesso completo à sua rotina.
            </p>
          </div>
        )}
          ...
          <div className="max-w-md mx-auto space-y-4">
            <div className="bg-surface-900/50 p-6 rounded-2xl border border-white/10 text-center">
              ...
              <div className="text-2xl font-bold text-white mt-1">
                {session.plan === 'PRO' ? 'PRO — R$ 29,90/mês' : 'VIP — R$ 19,90/mês'}
              </div>
            </div>
        ...
        {state === 'expired' && session.plan === 'VIP' && !session.usedUpgradeOffer && (
          <div className="max-w-lg mx-auto">
            ...
            <div className="relative rounded-[2rem] bg-gradient-to-b from-brand-900/20 to-[#050505] p-1 ring-1 ring-brand-500/30 shadow-[0_20px_40px_-10px_rgba(48,64,235,0.2)] flex flex-col mb-6">
              <div className="bg-[#050505]/80 backdrop-blur-xl rounded-[calc(2rem-4px)] p-8 text-center">
                <h2 className="text-3xl font-display font-bold text-white mb-2">PRO</h2>
                ...
                <p className="text-brand-200/80 mb-8">Experimente o plano PRO com desconto exclusivo no primeiro mês.</p>
                
                <button
                  onClick={() => handleUnlock('promo')}
                  disabled={loading}
                  className="w-full rounded-full bg-white text-[#050505] font-bold py-4 hover:bg-surface-100 transition-colors disabled:opacity-50"
                >
        ...
        {state === 'expired' && (session.plan === 'PRO' || session.usedUpgradeOffer) && (
          <div className="max-w-md mx-auto">
            <div className="bg-surface-900/50 p-8 rounded-[2rem] border border-white/10 text-center space-y-6 flex flex-col items-center">
              <h2 className="text-3xl font-display font-semibold text-white">{session.plan === 'PRO' ? 'PRO' : 'VIP'}</h2>
              <div className="flex items-baseline gap-1">
                <span className="text-4xl font-bold text-white">R$ {session.plan === 'PRO' ? '29,90' : '19,90'}</span>
                <span className="text-surface-500">/mês</span>
              </div>
              <button
                onClick={() => handleUnlock(session.plan === 'PRO' ? 'pro' : 'vip')}
                disabled={loading}
                className="w-full rounded-full bg-brand-500 hover:bg-brand-600 text-white font-bold py-4 transition-colors disabled:opacity-50"
              >
                {loading ? 'Redirecionando...' : `Renovar ${session.plan === 'PRO' ? 'PRO' : 'VIP'}`}
              </button>
            </div>
            <div className="text-center mt-6 pt-4 border-t border-white/10">
              <p className="text-surface-400 text-sm mb-4">Deseja mudar seu plano?</p>
              <button
                onClick={() => handleUnlock(session.plan === 'PRO' ? 'vip' : 'pro')}
                disabled={loading}
                className="rounded-xl bg-surface-800 hover:bg-surface-700 text-white py-3 px-6 text-sm font-medium transition-colors"
              >
                Mudar para {session.plan === 'PRO' ? 'VIP' : 'PRO'}
              </button>
            </div>
          </div>
        )}
